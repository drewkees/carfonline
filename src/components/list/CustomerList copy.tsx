...
const CustomerList: React.FC<CustomerListProps> = ({ onNewCustomer, onEditCustomer }) => {
  ...
  const [config, setConfig] = useState<{ customerSource: string }>({ customerSource: 'PROD' });
    ...
    <div className="min-h-screen flex flex-col">
      <main className="flex-1 p-4 sm:p-6">
        <div className="space-y-4 sm:space-y-6 pb-24">
          ...
          {/* Mobile Stacked View (first 4 columns) */}
<div className="block sm:hidden max-h-[calc(100vh-200px)] overflow-y-auto p-2 space-y-2">
  {currentCustomers.map((customer) => (
    <div
      key={customer.id}
      className="bg-card border border-border rounded-lg p-3 shadow-sm cursor-pointer hover:bg-muted/50"
      onDoubleClick={() => onEditCustomer(customer)}
    >
      {udfFields.filter(f => f.visible).slice(0, 4).map((field) => {
        const value = customer[field.fieldid];
        return (
          <div key={field.fieldid} className="flex justify-between py-1 border-b last:border-b-0 text-sm">
            <span className="font-medium text-muted-foreground">{field.fieldnames}</span>

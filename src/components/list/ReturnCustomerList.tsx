...
const ReturnCustomerList: React.FC<CustomerListProps>  = ({onEditCustomer }) => {
  ...
  const fetchUdfFields = async () => {
    try {
      const { data, error } = await supabase
        ...
        .eq('objectcode', 'returntomaker') // dynamic objectcode
  ...
  const fetchReturnCustomers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('customerdata')
        .select('*')
        .eq('approvestatus', 'RETURN TO MAKER')
        .order('datecreated', { ascending: false });
      if (error) throw error;
      setCustomers(data || []);
    } catch (error) {
      console.error('Error fetching return customers:', error);
      toast({ title: 'Error', description: 'Failed to fetch return customers.', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-xl font-semibold text-foreground">RETURNED CUSTOMERS</h2>
        </div>

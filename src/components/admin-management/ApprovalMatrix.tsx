...
function ApproverMultiSelect({
  ...
  return (
    <div className="flex flex-col gap-1">
      <label className="text-xs font-medium text-slate-500 dark:text-gray-400 uppercase tracking-wide">{label}</label>

      {/* Trigger row */}
      <button
        type="button"
        onClick={handleOpen}
        className="w-full px-3 py-2.5 rounded-lg bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-slate-900 dark:text-white text-sm flex justify-between items-center hover:border-gray-500 transition-colors min-h-[42px] text-left"
      >
        <span className={`truncate pr-2 ${selectedValues.length === 0 ? 'text-gray-500' : 'text-slate-900 dark:text-white'}`}>
        ...
        <span className="flex items-center gap-1.5 flex-shrink-0">
          ...
          {open ? <ChevronUp size={14} className="text-slate-500 dark:text-gray-400" /> : <ChevronDown size={14} className="text-slate-500 dark:text-gray-400" />}
        </span>
      </button>

      {/* Inline expanded panel — pushes content down, never clips */}
      {open && (
        <div className="rounded-lg border border-slate-300 dark:border-gray-600 bg-gray-850 overflow-hidden" style={{ background: '#1a1f2e' }}>
          {/* Search */}
          <div className="p-2 border-b border-slate-200 dark:border-gray-700">
            <input
              ...
              className="w-full px-3 py-1.5 rounded-md bg-slate-100 dark:bg-gray-900 border border-slate-200 dark:border-gray-700 text-slate-900 dark:text-white text-sm placeholder-gray-600 focus:outline-none focus:ring-1 focus:ring-blue-500 custom-scrollbar"
              value={search}
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors ${
                      selected ? 'bg-blue-600/20' : 'hover:bg-slate-100 dark:hover:bg-gray-700/60'
                    }`}
                  >
                    <div className={`w-4 h-4 rounded flex items-center justify-center border flex-shrink-0 transition-colors ${
                      selected ? 'bg-blue-600 border-blue-600' : 'border-slate-300 dark:border-gray-600'
                    }`}>
                      {selected && (
                        <svg className="w-2.5 h-2.5 text-slate-900 dark:text-white" fill="none" viewBox="0 0 10 8">
                          <path d="M1 4l3 3 5-6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      )}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-sm text-slate-900 dark:text-white font-medium truncate">{u.userid}</div>
                      <div className="text-xs text-slate-500 dark:text-gray-400 truncate">{u.fullname}</div>
                    </div>
                    {selected && (
          ...
          {selectedValues.length > 0 && (
            <div className="px-3 py-2 border-t border-slate-200 dark:border-gray-700 bg-slate-100/70 dark:bg-slate-100 dark:bg-gray-900/50">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs text-slate-500 dark:text-gray-400 font-medium">{selectedValues.length} selected</span>
                <button
                  onClick={() => onChange([])}
          ...
          {/* Close button */}
          <div className="px-3 py-2 border-t border-slate-200 dark:border-gray-700 flex justify-end">
            <button
              type="button"
              onClick={() => { setOpen(false); setSearch(''); }}
              className="text-xs text-slate-500 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white px-3 py-1 rounded-md hover:bg-slate-100/80 dark:hover:bg-gray-700/50 transition-colors"
            >
              Done
            </button>
          </div>
    ...
    <div className="h-full bg-background flex flex-col">
      {isMobile ? (
        ...
        <>
          ...
          <div className="flex-1 mx-4 mb-4 mt-4 overflow-hidden flex flex-col">
            <div className="flex-1 overflow-y-auto overflow-x-auto bg-white dark:bg-gray-800 rounded-lg shadow custom-scrollbar">
              <table className="min-w-full table-auto">
                <thead className="bg-slate-100 dark:bg-gray-900 sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="text-left px-6 py-4 text-sm font-semibold text-slate-800 dark:text-gray-200 whitespace-nowrap">APPROVAL TYPE</th>
                    <th className="text-left px-6 py-4 text-sm font-semibold text-slate-800 dark:text-gray-200 whitespace-nowrap">FIRST APPROVER</th>
                    <th className="text-left px-6 py-4 text-sm font-semibold text-slate-800 dark:text-gray-200 whitespace-nowrap">SECOND APPROVER</th>
                    <th className="text-left px-6 py-4 text-sm font-semibold text-slate-800 dark:text-gray-200 whitespace-nowrap">THIRD APPROVER</th>
                    <th className="text-left px-6 py-4 text-sm font-semibold text-slate-800 dark:text-gray-200 whitespace-nowrap">ACTIONS</th>
                  </tr>
                </thead>
                  ...
                  {filteredSchemas.length === 0 ? (
                    ...
                    filteredSchemas.map((schema) => (
                      <tr key={schema.id} className="hover:bg-slate-100/80 dark:hover:bg-gray-700/50 transition-colors cursor-pointer" onDoubleClick={() => handleEdit(schema)}>
                        <td className="px-6 py-4 text-slate-500 dark:text-gray-400 whitespace-nowrap text-sm">{(schema as any).company || '-'}</td>
                        <td className="px-6 py-4 text-slate-800 dark:text-gray-200 whitespace-nowrap">{schema.approvaltype}</td>
                        <td className="px-6 py-4 text-slate-800 dark:text-gray-200 whitespace-nowrap">{schema.firstapprover || '-'}</td>
                        <td className="px-6 py-4 text-slate-800 dark:text-gray-200 whitespace-nowrap">{schema.secondapprover || '-'}</td>
                        <td className="px-6 py-4 text-slate-800 dark:text-gray-200 whitespace-nowrap">{schema.thirdapprover || '-'}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-100 dark:bg-gray-900 border border-slate-200 dark:border-gray-700 rounded-xl w-full max-w-lg shadow-2xl flex flex-col" style={{ maxHeight: '92vh' }}>

            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-t-xl flex-shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center">
                  <GitMerge size={16} className="text-slate-900 dark:text-white" />
                </div>
                <div>
            ...
            <div className="px-6 py-5 space-y-5 overflow-y-auto flex-1 custom-scrollbar">
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      className="px-3 py-2.5 rounded-lg bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition appearance-none cursor-pointer"
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      className="px-3 py-2.5 rounded-lg bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition appearance-none cursor-pointer disabled:opacity-50"
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Click a level to expand it, search and tick users, then click <strong className="text-slate-500 dark:text-gray-400">Done</strong> to close.
                </p>
            ...
            {/* Footer */}
            <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200 dark:border-gray-700 bg-slate-100/60 dark:bg-gray-800/50 rounded-b-xl flex-shrink-0">

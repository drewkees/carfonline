...
export default function DashboardLayoutProps() {
  ...
  const fetchSchema = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('regionbu').select('*').order('id', { ascending: true });
           ...
           <table className="min-w-full table-auto">
            <thead className="bg-gray-900 sticky top-0 z-10">
              <tr>
                ...
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-200 whitespace-nowrap">DISRICT</th>

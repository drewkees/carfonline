...
export default function CCEMAIL() {
  ...
  const [newSchema, setNewSchema] = useState<
    Omit<Database['public']['Tables']['ccemail']['Insert'], 'ID'>
  >({
    email: '',
    customergroup: '',
    bc: '',
  ...
  const fetchSchema = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('ccemail').select('*').order('id', { ascending: true });
  ...
  const handleEdit = (schema) => {
    ...
    setNewSchema({
      email: schema.email || '',
      customergroup: schema.customergroup || '',
      bc: schema.bc || '',
      allcarf: schema.allcarf || false,
    });
    setShowModal(true);
  };
  ...
  const handleDelete = async (id) => {
    if (confirm('Are you sure you want to delete this CC email?')) {
      const { error } = await supabase.from('ccemail').delete().eq('id', id);
      if (error) {
        toast({
          title: 'Error',
  ...
  const filteredSchemas = schemas.filter((schema) => {
    ...
    return (
      ...
      schema.customergroup?.toLowerCase().includes(q) ||
      schema.bc?.toLowerCase().includes(q)
    );
  });
      ...
      {isMobile ? (
          ...
          <div className="flex-shrink-0 bg-background border-b border-border">
            <div className="flex flex-col items-start justify-between gap-3 p-4 pb-3">
              <div className="flex items-center justify-between w-full">
                <h2 className="text-lg font-semibold text-foreground">CC EMAILS</h2>
                <button
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div>
                          ...
                          <div className={`text-sm mt-0.5 ${schema.allcarf ? 'text-green-500' : 'text-gray-500'}`}>
                            {boolText(schema.allcarf)}
                          </div>
                        </div>
                      </div>
                    </CardContent>
        ...
        <>
          <div className="flex items-center justify-between px-4 py-4 bg-background border-b border-slate-200 dark:border-gray-700">
            <h2 className="text-xl font-semibold text-foreground">CC Emails</h2>
              ...
              <table className="min-w-full table-auto">
                <thead className="bg-slate-100 dark:bg-gray-900 sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="text-left px-6 py-4 text-sm font-semibold text-slate-800 dark:text-gray-200 whitespace-nowrap">CUSTOMER GROUP</th>
                    <th className="text-left px-6 py-4 text-sm font-semibold text-slate-800 dark:text-gray-200 whitespace-nowrap">BUSINESS CENTER</th>
                    <th className="text-left px-6 py-4 text-sm font-semibold text-slate-800 dark:text-gray-200 whitespace-nowrap">ALL CARF</th>
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-gray-700">
                  {filteredSchemas.map((schema) => (
                    <tr key={schema.id} className="hover:bg-slate-100/80 dark:hover:bg-gray-700/50 transition-colors cursor-pointer" onDoubleClick={() => handleEdit(schema)}>
                      ...
                      <td className="px-6 py-4 text-slate-800 dark:text-gray-200 whitespace-nowrap">{schema.customergroup}</td>
                      <td className="px-6 py-4 text-slate-800 dark:text-gray-200 whitespace-nowrap">{schema.bc}</td>
                      <td className="px-6 py-4 text-slate-800 dark:text-gray-200 whitespace-nowrap">{boolText(schema.allcarf)}</td>
          ...
          <div className="w-[95vw] max-w-3xl rounded-xl border border-slate-200 dark:border-gray-700 bg-slate-100 dark:bg-gray-900 text-slate-900 dark:text-gray-100 shadow-2xl flex flex-col">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-semibold text-slate-900 dark:text-white">
                    {editingSchema ? 'Edit CC Email' : 'Add CC Email'}
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-gray-400">Configure carbon copy recipients</p>
                </div>
              </div>
              <button
            ...
            <div className="px-6 py-5 grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                ...
                { key: 'bc', label: 'Business Center', type: 'text' }
              ].map(({ key, label, type }) => (
              ...
              <div className="flex items-center gap-2 pt-2 md:col-span-2">
                <input
                  type="checkbox"
                  id="allcarf"
                  checked={newSchema.allcarf || false}
                  onChange={(e) =>
                    setNewSchema({ ...newSchema, allcarf: e.target.checked })
                  }
                ...
                />
                <label htmlFor="allcarf" className="text-sm cursor-pointer">
                  All CARF
                </label>
              </div>
            </div>

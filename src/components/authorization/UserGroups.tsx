...
export default function UserGroups() {
  ...
  const [newSchema, setNewSchema] = useState<
    Omit<Database['public']['Tables']['usergroups']['Insert'], 'ID'>
  >({
    groupcode: '',
    groupname: '',
  });
  ...
  const fetchSchema = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('usergroups').select('*').order('id', { ascending: true });
  ...
  const handleSaveSchema = async () => {
    try {
      ...
      toast({
        title: 'Error',
        description: 'Failed to save user group',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleDelete = async (id) => {
    if (confirm('Are you sure you want to delete this user group?')) {
      const { error } = await supabase.from('usergroups').delete().eq('id', id);
      if (error) {
        toast({
          title: 'Error',
        ...
        toast({ title: 'Deleted', description: 'User group deleted successfully' });
      }
    }
  };
  ...
  const filteredSchemas = schemas.filter((schema) => {
    ...
    return (
      schema.groupcode?.toLowerCase().includes(q) ||
      schema.groupname?.toLowerCase().includes(q)
    );
  });
    ...
    <div className="h-full bg-background flex flex-col">
            ...
            <div className="flex-1 overflow-auto bg-white dark:bg-gray-800 rounded-lg shadow">
              <table className="min-w-full table-auto">
                <thead className="bg-slate-100 dark:bg-gray-900 sticky top-0 z-10">
                  <tr>
                    <th className="text-left px-6 py-4 text-sm font-semibold text-slate-800 dark:text-gray-200 whitespace-nowrap">GROUP CODE</th>
                    <th className="text-left px-6 py-4 text-sm font-semibold text-slate-800 dark:text-gray-200 whitespace-nowrap">GROUP NAME</th>
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-gray-700">
                  {filteredSchemas.map((schema) => (
                    <tr key={schema.id} className="hover:bg-slate-100/80 dark:hover:bg-gray-700/50 transition-colors">
                      <td className="px-6 py-4 text-slate-800 dark:text-gray-200 whitespace-nowrap">{schema.groupcode}</td>
                      <td className="px-6 py-4 text-slate-800 dark:text-gray-200 whitespace-nowrap">{schema.groupname}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-[95vw] max-w-2xl rounded-xl border border-slate-200 dark:border-gray-700 bg-slate-100 dark:bg-gray-900 text-slate-900 dark:text-gray-100 shadow-2xl flex flex-col">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-semibold text-slate-900 dark:text-white">
                    {editingSchema ? 'Edit User Group' : 'Add User Group'}
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-gray-400">Manage user group configuration</p>
                </div>
              </div>
              <button
            ...
            <div className="px-6 py-5 grid grid-cols-1 md:grid-cols-2 gap-3">
              {['groupcode', 'groupname'].map((field) => (
                <div key={field} className="flex flex-col">
                  <label className="text-sm mb-1 capitalize">
                    {field === 'groupcode' ? 'Group Code' : 'Group Name'}
                  </label>
                  <input
                    type="text"
